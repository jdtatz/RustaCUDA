...
use cuda_sys::{self as cuda, CUarray, CUarray_format, CUarray_format_enum};
...
impl ArrayObject {
    ...
    pub fn from_descriptor(descriptor: &ArrayDescriptor) -> CudaResult<Self> {
        ...
        unsafe { cuda::cuArray3DCreate_v2(handle.as_mut_ptr(), &descriptor.desc) }
            .to_result()?;
        Ok(Self {
    ...
    pub fn descriptor(&self) -> CudaResult<ArrayDescriptor> {
        ...
        unsafe {
            cuda::cuArray3DGetDescriptor_v2(raw_descriptor.as_mut_ptr(), self.handle)
        }
        .to_result()?;
